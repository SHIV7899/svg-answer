...
BOT_OWNER_ROLE = 'Y.A.S.P. BOT' # change to what you need
#BOT_OWNER_ROLE_ID = "834718012740861952"
...
class Bot(discord.Client):
    ...
    def __init__(self, answer_scores):
        ...
        # embed creation
        self.embed=discord.Embed(title="⚠//Y.A.S.P.//⚠POWER⚠", description="**Web Searching** :spy:")
    ...
    async def on_ready(self):
        ...
        await self.change_presence(activity=discord.Game(name='Trivia with ADMINISTRATIVE *help'))
    ...
    async def on_message(self, message):
        ...
        if message.content.startswith('*help'):
          ...
          if BOT_OWNER_ROLE in [role.name for role in message.author.roles]:
           ...
           embed.add_field(name="Support Game", value="**HQ Trivia**", inline=False)
           embed.add_field(name="when Question come put command", value=" `.H` is command work for all support game\n**FOR MORE INFO CONTACT TO //SIMBHA//#3664**", inline=False)
...
def bot_with_cyclic_update_process(update_event, answer_scores):
    ...
    loop.create_task(bot.start('ODM1ODI2MjcyMTg1NjE0MzU2.YIVF4A.32cryB2tLYZj7J9B15Tq3L9GZco'))
    loop.run_forever()
...
def selfbot_process(update_event, answer_scores):
    ...
    loop = asyncio.get_event_loop()
    loop.create_task(selfbot.start('NzIyODY0MTQzMTUzMjM0MDUx.X3uygw.J-NCospMLYMHNZG9QtxHTGb24jY'))
